            ...
            if(chat.member_id == "SYSTEM") {
                ...
                <div class="group-chat-message">
                    <div class="group-message">
                        ...
                        <div class="group-chat-message-contents" ${chat.is_pinned ? 'style="border-style:dashed;border-color:#505050;background:#222"' : ''}>
...
$('#leave-room').on('click', e => {
    ...
    $.post("https://qb-phone/LeaveGroupChat", JSON.stringify({roomID: currentRoom, citizenid: cid}), function(status) {
        if(status) {
            ...
            QB.Phone.Notifications.Add("fa fa-check", "Discordia", "You have left #" + slug(getChatRoomData().room_name), "#1DA1F2", 2500)
        }  
    })
})
...
$('#join-room').on('click', e => {
    ...
    $.post("https://qb-phone/JoinGroupChat", JSON.stringify({roomID: currentRoom}), function(status) {
        if(status) {
            ...
            QB.Phone.Notifications.Add("fa fa-check", "Discordia", "You are now a member of #" + slug(getChatRoomData().room_name), "#1DA1F2", 2500)
        }
    })
})
...
function openChatRoom(id) {
    ...
    if(data.is_masked && ! QB.Phone.Data.IsHacked) {
        QB.Phone.Notifications.Add("fa fa-times", "Discordia", "This room is using a private VPN, your phone cannot connect.", "#1DA1F2", 2500)
    ...
    if( ! isUserAnOwner(room)) {
        ...
        $.post("https://qb-phone/DeactivateRoom", JSON.stringify({roomID: room}), function(status) {
            if(status) {
                ...
                QB.Phone.Notifications.Add("fa fa-check", "Discordia", "You have deactivated the room.", "#1DA1F2", 2500)

            } else {
                QB.Phone.Notifications.Add("fa fa-times", "Discordia", "Failed to deactivate the room.", "#1DA1F2", 2500)
            }
        })       
    }  
})
        ...
        if(pinOk) {
            $.post("https://qb-phone/ChangeRoomPin", JSON.stringify({pinCode: pin, roomID: id}), function(status) {
                if(status) {
                    $('#change-pin').val(pin)
                    $('#owned-rooms-list-close').trigger('click')
                    QB.Phone.Notifications.Add("fa fa-check", "Discordia", "Room passcode successfully changed!", "#1DA1F2", 2500)
                } else {
                    QB.Phone.Notifications.Add("fa fa-times", "Discordia", "Failed to change room passcode.", "#1DA1F2", 2500)
                }
            })
        } else {
            QB.Phone.Notifications.Add("fa fa-times", "Discordia", "Your new passcode was not accepted, must be less than 50 characters.", "#1DA1F2", 2000)
        }
    }
})
    ...
    if(e.which == 13) {
        ...
        if(chatroom) {
            if(chatroom.is_masked && ! QB.Phone.Data.IsHacked) {
                ...
                QB.Phone.Notifications.Add("fa fa-times", "Discordia", "You are already the owner of this room.", "#1DA1F2", 4000)
                ...
                QB.Phone.Notifications.Add("fa fa-times", "Discordia", "You are already a member of this room.", "#1DA1F2", 4000)
                openChatRoom(chatroom.id)
                $('.room-input-code').val("")
                ...
                $.post('https://qb-phone/JoinGroupChat', JSON.stringify({roomID: id}), () => {
                    openChatRoom(chatroom.id)
                    QB.Phone.Notifications.Add("fa fa-check", "Discordia", "You are now a member of #" + slug(chatroom.room_name), "#1DA1F2", 4000)
    ...
    if(e.which == 13) {
        ...
        $.post('https://qb-phone/JoinGroupChat', JSON.stringify({roomID: id, roomPin: pin}), (status) => {
            if(status) {
                ...
                QB.Phone.Notifications.Add("fa fa-times", "Discordia", "That pin code is incorrect.", "#1DA1F2", 4000)
            }
        })  
    }
})
...
$(document).on('click', '.pin-message', function(e) {
    ...
    $.post("https://qb-phone/ToggleMessagePin", JSON.stringify({roomID: room, messageID: message}), function(status) {
        if( ! status) {
            QB.Phone.Notifications.Add("fa fa-times", "Discordia", "You don't have permission to do that.", "#1DA1F2", 4000)
        } else {
            QB.Phone.Notifications.Add("fa fa-info", "Discordia", "You have changed the pinned status.", "#1DA1F2", 4000)
...
$(document).on('click', '#remove-member', function(e){
    ...
    $.post("https://qb-phone/LeaveGroupChat", JSON.stringify({roomID: room, citizenid: cid}), function(status) {
        if(status) {
            ...
            $('#members-back-btn').trigger('click')
            QB.Phone.Notifications.Add("fa fa-check", "Discordia", "You have removed a member.", "#1DA1F2", 2500)
        } 
    })
})